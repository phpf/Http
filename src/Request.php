...
class Request {
	...
	/**
	 * File extensions to strip from the end of URLs, delimeted with "|".
	 * 
	 ...
	 * @var string
	 */
	protected static $file_extensions = 'html|jsonp|json|xml';
	 ...
	 * @var array
	 */
	protected static $allow_method_override = array(
		'header' => true,
		'parameter' => false,
	);
	 ...
	 * @var array
	 */
	protected static $allow_content_types = array(
		'html' => true, 
		'json' => true, 
		'jsonp' => true, 
		'xml' => true,
	 ...
	 */
	public function __construct($method, $uri, $query, array $headers, $body = null, array $cookies = null, array $files = null){
		
		// Clean and set request path
		$uri = trim(filter_var($uri, FILTER_SANITIZE_STRING), '/');
		
		// match file extensions to set content-type
		$extensions = static::$file_extensions;
		if (preg_match("/[\.|\/]($extensions)/", $uri, $matches)) {
		...
		if (! empty($this->body)) {
			...
			if (isset($this->headers['content-type']) 
				&& false !== strpos($this->headers['content-type'], 'www-form-urlencoded')
			) {
				parse_str($body, $this->body_params);
				$this->params = array_merge($this->params, $this->body_params);
			}
		}
		...
		if ('POST' === $method) {
			// via header
			if (isset($this->headers['x-http-method-override']) && $this->isMethodOverrideAllowed('header')) {
				$method = $this->headers['x-http-method-override']; 
			}
			//via parameter
			if (isset($this->query_params['_method']) && $this->isMethodOverrideAllowed('parameter')) {
	 ...
	 * @return $this
	 */
	public function setPathParams(array $params = null){
	 ...
	 * @return $this
	 */
	public function setSession(\Phpf\Session\Session $session) {
	...
	public function setContentType($type) {
		...
		if (isset(static::$allow_content_types[$type])) {
	...
	/**
	 * Returns true if content type is set.
	 * 
	 * @return boolean True if content-type set, otherwise false.
	 */
	...
	public function isContentTypeAllowed($type) {
		return isset(static::$allow_content_types[$type]);
	}
	...
	public function getAllowedContentTypes() {
		return array_keys(static::$allow_content_types);
	}
	...
	public function isMethodOverrideAllowed($where) {
		return static::$allow_method_override[$where];
	}
